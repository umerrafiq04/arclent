...
# The active LLM provider for backend/agent/llm.py — "groq", "deepseek", or "mistral". Groq is the
# default now — both Mistral (subscription tier stopped allowing MISTRAL_MODEL) and DeepSeek
# (account ran out of balance) hit account-level outages in production; Mistral/DeepSeek stay
# fully wired so switching back is a one-variable rollback, not a code change, once either is
# resolved.
